...
st.title("📷 YOLO Real-time Camera Detection")
    ...
    if uploaded_file is not None:
        ...
        if len(detections) > 0:
            for box in detections:
                ...
                print(class_name)
                # Check if person with helmet/hard hat is detected
                if ('person' in class_name) and \
                   ('hard hat' in class_name):
                    helmet_detected = True
                # Check if person without helmet is detected
                elif ('person' in class_name) and \
                     ('hard hat' not in class_name):
        ...
        if no_helmet_detected:
            safety_status_text.error("🚨 **Image Result : PELANGGARAN: APD TIDAK LENGKAP.** Terdeteksi Pekerja Tanpa Helm.")
        elif helmet_detected:
            safety_status_text.success("✅ **AMAN: K3 Terpenuhi**")
        else:
            safety_status_text.info("📌 Tidak ada deteksi")

        # Display detection count
        fps_text.info(f"🔍 Detections: {len(detections)}")
    ...
    while st.session_state.run:
        ...
        if len(detections) > 0:
            for box in detections:
                ...
                # Check if person with helmet/hard hat is detected
                if ('person' in class_name or 'worker' in class_name or 'pekerja' in class_name) and \
                   ('helmet' in class_name or 'hard hat' in class_name or 'hardhat' in class_name):
                    helmet_detected = True
                # Check if person without helmet is detected
                elif ('person' in class_name or 'worker' in class_name or 'pekerja' in class_name):
                    no_helmet_detected = True

        # Check safety status for display
        if no_helmet_detected:
        ...
        # Display safety status below FPS
        if safety_status == "violation":
            safety_status_text.error("🚨 **PELANGGARAN: APD TIDAK LENGKAP.** Terdeteksi Pekerja Tanpa Helm.")
